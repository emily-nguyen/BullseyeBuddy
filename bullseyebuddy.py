...
for sound_file in search_strings:
  with open(sound_file, 'rb') as audio_file:
    ...
    if 'cartwheel' in text:
      all_offers = req_cart.findAll('div', class_="large-3")
      first_offer = all_offers[0].get('id')
      second_offer = all_offers[1].get('id')
      third_offer = all_offers[2].get('id')
      
      speech = "hi"

      pass
    else:
      first_link = req_item.find('a', class_="productClick")
